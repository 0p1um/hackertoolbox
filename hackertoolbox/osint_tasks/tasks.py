...
def advanced_crawler(url, depth=1, mobile_emulation=False, allow_external=False):
    ...
    for i in range(depth):
        ...
        for url in links:
            ...
            result["data"].append(
                {"url": url, "source_code": source_code, "links": links}
            )
...
def pgp_search(querry):
    ...
    for s in d:
        try:
            res = (
                request.urlopen(
                    "https://" + s + "/pks/lookup?search=" + q + "&op=index"
                )
                .read()
                .decode("utf-8")
            )
            ...
            for element in res:
                ...
                data["data"].append(
                    {"name": name, "email": email, "public_key_fingerprint": public_key}
                )
    ...
    try:
        result = [
            ...
            for item in json.load(
                request.urlopen("https://crt.sh/?q=%" + keyword + "&output=json")
            )
        ]
        result.extend(
            [
    ...
    for record_type in record_types_to_query:
        try:
            if record_type == "CNAME":
                ...
                [
                    res["data"].append(
                        {"query": str(query), "record_type": "CNAME", "result": str(e)}
                    )
                    for e in result
                ]
                while len(result) >= 1:
