...
def refresh_datasets(request):
    for dir in listdir(settings.DATASETS_PATH):
        ...
        dataset = Dataset(name=info['name'], date=info['date'], cipher_format=info['format'], compromised_data=info['compromised_data'], description=info['description'], path=dataset_path)
