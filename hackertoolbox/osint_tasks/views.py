...
from .models import Task, GoogleSearch, Job, ResultFromTask, PgpSearch, Item, AdvancedCrawler, CertificateTransparency, DnsLookup, SimpleCrawler, ShodanSearch
...
def get_config(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            with open(settings.TASKS_CONF_FILE, 'w') as file:
                ...
                file.write('SHODAN_API_KEY='+form.cleaned_data['shodan_api_key']+'\n')
    ...
    # if a GET (or any other method) we'll create a blank form
        ...
        form = ConfigForm(initial={'google_api': google_api, 'google_cse_id':google_cse_id, 'shodan_api_key':shodan_api_key})
...
class ResultsListView(ListView):
    ...
    def get_queryset(self):     # ajouter get_context_data afin d'avoir la pagination de possible ?
...
class GoogleSearchCreate(CreateView):
    model = GoogleSearch
    fields = ['name','description','query','num', 'date_restrict_d', 'exact_terms', 'file_type', 'gl', 'lr', 'or_terms', 'related_site', 'sort', 'site_search', 'search_type']
    template_name = 'generic_form.html'
...
class AdvancedCrawlerCreate(CreateView):
    ...
    template_name = 'generic_form.html'
    fields = ['name', 'description', 'url', 'depth', 'mobile_emulation', 'allow_external']
