...
crontab_validator = RegexValidator(
    r"([0-9]+-[0-9]+|[\*]+\/[0-9]+|[\*])", "Please enter a valid crontab style entrie"
)
...
class Task(PolymorphicModel):
    ...
    description = models.TextField(
        max_length=500, blank=True, verbose_name="Description of the job (optional)"
    )
...
class Job(PolymorphicModel):
    ...
    minute = models.CharField(
        max_length=6,
        default="*",
        verbose_name="The minute(s) at which the command will be executed. (0-59, ranges, or divided, *=all)",
        validators=[crontab_validator],
    )
    hour = models.CharField(
        max_length=6,
        default="*",
        verbose_name="The hour(s) at which the command will be executed. (0-23, ranges, or divided, *=all)",
        validators=[crontab_validator],
    )
    day_of_the_month = models.CharField(
        max_length=6,
        default="*",
        verbose_name="The day(s) of the month on which the command will be executed. (1-31, ranges, or divided, *=all)",
        validators=[crontab_validator],
    )
    month_of_the_year = models.CharField(
        max_length=6,
        default="*",
        verbose_name="The month(s) of the year during which the command will be executed. (1-12, ranges, or divided, *=all)",
        validators=[crontab_validator],
    )
    day_of_the_week = models.CharField(
        max_length=6,
        default="*",
        verbose_name="The day(s) of the week on which the command will be executed. (0-7, 7=Sun or use names, ranges, or divided, *=all)",
        validators=[crontab_validator],
    )
    ...
    def create_periodic_task(self):
        if self.is_active:
            schedule, _ = CrontabSchedule.objects.get_or_create(
                minute=self.minute,
                hour=self.hour,
                day_of_week=self.day_of_the_week,
                day_of_month=self.day_of_the_month,
                month_of_year=self.month_of_the_year,
            )
...
class GoogleSearch(Task):
    ...
    num = models.PositiveIntegerField(
        ...
        null=True,
        default=10,
        verbose_name="Number of result to keep, 10 maximum for free users",
    )
    date_restrict_d = models.PositiveIntegerField(
        blank=True,
        null=True,
        verbose_name="Date restriction: number of days old maximum for results",
    )
    exact_terms = models.CharField(
        null=True,
        max_length=1000,
        blank=True,
        verbose_name="Exact terms to include in the search",
    )
    file_type = models.CharField(
        ...
        blank=True,
        verbose_name="File type searched",
    )
    gl = models.CharField(
        null=True,
        max_length=2,
        choices=GL_CHOICES,
        blank=True,
        verbose_name="Geo location of the search request",
    )
    link_site = models.URLField(
        null=True,
        max_length=2000,
        blank=True,
        verbose_name="Link that must be in the search results",
    )
    lr = models.CharField(
        null=True,
        max_length=10,
        choices=LR_CHOICES,
        blank=True,
        verbose_name="Language restriction: results must be in that language",
    )
    or_terms = models.CharField(
        null=True,
        max_length=1000,
        blank=True,
        verbose_name="Additional query field with OR operator",
    )
    related_site = models.URLField(
        null=True,
        max_length=2000,
        blank=True,
        verbose_name="URL, results must be related to it.",
    )
    sort = models.CharField(
        ...
        blank=True,
        verbose_name="Results are sort by it",
    )
    site_search = models.CharField(
        null=True,
        max_length=255,
        blank=True,
        verbose_name="Particular domain, results must be from this domain",
    )
    search_type = models.CharField(
        ...
        blank=True,
        verbose_name="Search type: Webpage or Images",
    )
...
class ItemGoogleSearch(Item):
    ...
    def list_of_subitems(self):
        list_of_subitems = [
            {
                "date": self.result_gs.date_done,
                "type": "Google Search link",
                "abstract_type": "url",
                "data": self.link,
...
class ItemPgpSearch(Item):
    ...
    def list_of_subitems(self):
        list_of_subitems = [
            {
                "date": self.result_pgps.date_done,
                "type": "pgp search mail",
                "abstract_type": "email",
                "data": self.email,
                "transform": ["google_search"],
            },
            ...
            {
                "date": self.result_pgps.date_done,
                "type": "pgp search pub key fingerprint",
                "abstract_type": "name",
                "data": self.public_key_fingerprint,
                "transform": ["google_search"],
            },
        ]
        return list_of_subitems
...
class AdvancedCrawler(Task):
    ...
    url = models.URLField()
    mobile_emulation = models.BooleanField(
        default=False,
        verbose_name="Enable emulation of mobile to gather mobile version website ( emulated device is Nexus5 )",
    )
    depth = models.PositiveIntegerField(
        default=1,
        validators=[MaxValueValidator(50), MinValueValidator(1)],
        verbose_name="Depth of the crawling",
    )
    allow_external = models.BooleanField(
        default=False,
        verbose_name="Allow the crawler to crawl external website referenced (urls in other domain that the url field)",
    )
    ...
    def args(self):
        return json.dumps(
            [self.url, self.depth, self.mobile_emulation, self.allow_external]
        )

    def run(self):
        advanced_crawler.delay(
            self.url, self.depth, self.mobile_emulation, self.allow_external
        )
...
class ItemAdvancedCrawler(Item):
    ...
    def list_of_subitems(self):
        list_of_subitems = [
            {
                "date": self.result_ac.date_done,
                "type": "advanced crawler source code",
                "abstract_type": "text",
                "data": self.source_code,
            },
            {
                "date": self.result_ac.date_done,
                "type": "advanced crawler url",
                "abstract_type": "url",
                "data": self.url,
            },
        ]
        for link in ast.literal_eval(self.links_list):
            list_of_subitems.append(
                {
                    "date": self.result_ac.date_done,
                    "type": "advanced crawler url",
                    "abstract_type": "url",
...
class ItemCertificateTransparency(Item):
    ...
    result_cts = models.ForeignKey(
        ResultCertificateTransparency, on_delete=models.CASCADE
    )
    ...
    def list_of_subitems(self):
        list_of_subitems = [
            {
                "date": self.result_cts.date_done,
                "type": "Certificate transparency domain",
                "abstract_type": "domain",
                "data": self.domain,
                "transform": [
                    "advanced_crawler",
                    "ct_search",
...
class DnsLookup(Task):
    ...
    type_CNAME = models.BooleanField(
        default=False,
        verbose_name="CNAME records lookup (recursive, will include all redirection)",
    )
    ...
    type_ALL = models.BooleanField(
        default=False,
        verbose_name="All types of records lookup, require a good understanding of dns records",
    )
...
class ItemDnsLookup(Item):
    ...
    def list_of_subitems(self):
        if self.record_type == "A" or self.record_type == "AAAA":
            list_of_subitems = [
                {
                    "date": self.result_dl.date_done,
                    "type": "DNS " + self.record_type,
                    "abstract_type": "IP",
                    "data": self.result,
...
class ItemSimpleCrawler(Item):
    ...
    def list_of_subitems(self):
        list_of_subitems = [
            {
                "date": self.result_sc.date_done,
                "type": "Simple Crawler url",
                "abstract_type": "url",
                "data": self.url,
            ...
            {
                "date": self.result_sc.date_done,
                "type": "Simple Crawler source code",
                "abstract_type": "text",
                "data": self.source_code,
                "transform": [],
            },
...
class ItemShodanSearch(Item):
    ...
    def list_of_subitems(self):
        list_of_subitems = [
            {
                "date": self.result_ss.date_done,
                "type": "Shodan Search IP",
                "abstract_type": "IP",
                "data": self.ip,
                "transform": ["advanced_crawler", "google_search"],
            },
            ...
            {
                "date": self.result_ss.date_done,
                "type": "Shodan Search location",
                "abstract_type": "location",
                "data": self.location,
                "transform": [],
            },
            {
                "date": self.result_ss.date_done,
                "type": "Shodan Search port",
                "abstract_type": "port",
                "data": self.port,
                "transform": [],
            },
